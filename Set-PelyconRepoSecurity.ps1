...
$TemplateMap = @(
    @{
        ...
        RepoPath = "CLAUDE.md"
        CommitMessage = "Add Pelycon CLAUDE.md security rules"
    },
    @{
        TemplateNames = @("security.yml")
        RepoPath = ".github/workflows/security.yml"
        CommitMessage = "Add Pelycon security workflow"
    },
    @{
        TemplateNames = @("gitleaks.toml", ".gitleaks.toml")
        RepoPath = ".gitleaks.toml"
        CommitMessage = "Add Pelycon Gitleaks config"
    },
    @{
        TemplateNames = @("gitleaksignore", ".gitleaksignore")
        RepoPath = ".gitleaksignore"
...
try {
    ...
    Write-Host "  - Template names should be visible: CLAUDE.md, security.yml, gitleaks.toml, gitleaksignore."
    exit 1
}
